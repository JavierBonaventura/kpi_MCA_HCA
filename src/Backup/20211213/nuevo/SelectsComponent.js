...
const SelectsComponent = ({
  ...
  const handleDuctoChange = (event) => {
    ...
    setductoSeleccionado(event.target.value);
  };

  const handleTramoChange = (event) => {
    setSelectedTramo(event.target.value);
    settramoSeleccionado(event.target.value);
  };


  useEffect(() => {
    console.log("tramoSeleccionado actualizado:", tramoSeleccionado);
    console.log("ductoSeleccionado actualizado:", ductoSeleccionado);
  }, [tramoSeleccionado, ductoSeleccionado]);
          ...
          >
            ...
            {selectedDucto && !selectedEstudio
              ...
              : // Si ninguno de los dos tiene valor, usar todosLosestudios
                [
                  ...new Set(todosLosestudios.map((e) => e.Assessment_Name)),
                ].map((estudio, index) => (
                  <option key={index} value={estudio}>
                    {estudio}
                  </option>
