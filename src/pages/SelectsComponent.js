  ...
  const agregarNombreDucto = (todosLosPipelines, filteredDuctos) => {
    ...
    Object.keys(pipelinesConNombreDucto).forEach((assessmentName) => {
      ...
      pipelines.forEach((pipeline) => {
        ...
        const ducto = filteredDuctos.find((ducto) => ducto.Pipeline === pipeline.Pipeline);
        if (ducto) {
  ...
  useEffect(() => {
    const fetchAllPipelines = async () => {
      ...
      for (const assessment of approvedAssessments) {
        try {
          const ducts = await fetchPipelinesByAssessment(assessment.Assessment_Name);
          pipelines[assessment.Assessment_Name] = ducts;  // Guardamos los ductos para esta evaluación
        } catch (error) {
          console.error("Error fetching pipelines for assessment:", assessment.Assessment_Name, error);
        }
      }

      // Actualizamos el estado con los ductos obtenidos
      const pipelinesConNombreDucto = agregarNombreDucto(pipelines, filteredDuctos);
...
>
  ...
  {selectedAssessment
    ? [
        ...new Set(
          todosLosPipelines[selectedAssessment]?.map((ducto) => ducto.DuctoName)
        ),
      ].map((ductoName, index) => (
