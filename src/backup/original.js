...
document.getElementById("tspan172-3-5-25").textContent = attributesObject["FYEH_FQYE01"];
document.getElementById("tspan172-3-5-25-0").textContent = attributesObject["HER_FQY_015"];
document.getElementById("tspan172-3").textContent = attributesObject["IMP_NACO_PC"];
document.getElementById("tspan172-3-5-2-2").textContent = attributesObject["NAC_PT_903"];
              ...
              if (dataItem.length > 1) {
                  ...
                  if (value !== null && value !== '') {
                      ...
                      if (!isNaN(value)) {
                          ...
                          if (name === "Tiempo a Mínimo Inventario" || name === "Tiempo a Máximo Inventario") {
...
document.getElementById("tspan172-3-5-2-2-0").textContent = outputObject["VS_ZARAGOZA_MLV-037"] || "";
document.getElementById("tspan172-3-5-2-2-0-6").textContent = outputObject["VS_CANANEA_MLV-057"] || "";
document.getElementById("tspan172-3-7").textContent = outputObject["CUI_MLV-078"] || "";
document.getElementById("tspan172-3-7-6").textContent = outputObject["VS_JACALITOS_MLV-101"] || "";
document.getElementById("tspan172-3-7-3").textContent = outputObject["VS_IMURIS_MVL-125"] || "";
document.getElementById("tspan172-3-7-1").textContent = outputObject["VS_MAGDALENA_MLV"] || "";
document.getElementById("tspan172-3-5-2-2-0-6-3").textContent = outputObject["ANA_MLV-168"] || "";
document.getElementById("tspan172-3-7-1-2-2").textContent = outputObject["VS_LLANO_MLV-186"] || "";
document.getElementById("tspan172-3-7-1-2-1").textContent = outputObject["VS_BENAJMIN_MLV-206"] || "";
document.getElementById("tspan172-3-7-1-2-3").textContent = outputObject["VS_TINAJAS_MLV-226"] || "";
document.getElementById("tspan172-3-5-2-2-0-6-3-3").textContent = outputObject["CHI_MLV-249"] || "";
document.getElementById("tspan172-3-7-1-2-2-2").textContent = outputObject["VS_SELVA_MLV-275"] || "";
document.getElementById("tspan172-3-7-1-2-1-4").textContent = outputObject["VS_PESQUERIA_MLV-296"] || "";
document.getElementById("tspan172-3-7-1-2-3-5").textContent = outputObject["VS_SANPEDRO_MLV-319"] || "";
document.getElementById("tspan172-3-0").textContent = outputObject["IMP_NACO"] || "";
document.getElementById("tspan172-3-8-5").textContent = outputObject["Empaque"] || "";
document.getElementById("tspan172-3-8").textContent = outputObject["Tiempo a Máximo Inventario"] !== undefined ? outputObject["Tiempo a Máximo Inventario"] : "";
document.getElementById("tspan172-3-8-1").textContent = outputObject["Tiempo a Mínimo Inventario"] !== undefined ? outputObject["Tiempo a Mínimo Inventario"] : "";
 ...
 function cambiarValor(key, elementId) {
   ...
   if (nuevoValor !== null && nuevoValor.trim() !== "") {
     ...
     if (isValidNumber) {
       ...
       attributeService.saveEntityAttributes(params.entityId, "SERVER_SCOPE", attributesToSave).subscribe(
         () => {
...
document.getElementById("text172-10-8").addEventListener("click", () => cambiarValor("FYEH_FQYE01", "tspan172-3-5-25"));
document.getElementById("text172-10-4").addEventListener("click", () => cambiarValor("HER_FQY_015", "tspan172-3-5-25-0"));
document.getElementById("text172-10").addEventListener("click", () => cambiarValor("IMP_NACO_PC", "tspan172-3"));
document.getElementById("text172-10-5").addEventListener("click", () => cambiarValor("NAC_PT_903", "tspan172-3-5-2-2"));
            ...
            attributes.forEach((attribute) => {
                ...
                if (oldValue !== newValue) {
                    ...
                    if (attribute.key === "HER_FQY_015") {
                        document.getElementById("tspan172-3-5-25-0").textContent = newValue;
                    }
                    if (attribute.key === "IMP_NACO_PC") {
                        document.getElementById("tspan172-3").textContent = newValue;
                    }
                    if (attribute.key === "NAC_PT_903") {
                        document.getElementById("tspan172-3-5-2-2").textContent = newValue;
                    }
                    if (attribute.key === "VS_ZARAGOZA_MLV-037") {
                        document.getElementById("tspan172-3-5-2-2-0").textContent = newValue;
                    }
                    if (attribute.key === "VS_CANANEA_MLV-057") {
                        document.getElementById("tspan172-3-5-2-2-0-6").textContent = newValue;
                    }
                    ...
                    if (attribute.key === "ANA_MLV-168") {
                        document.getElementById("tspan172-3-5-2-2-0-6-3").textContent = newValue;
                    }
                    if (attribute.key === "VS_LLANO_MLV-186") {
                        document.getElementById("tspan172-3-7-1-2-2").textContent = newValue;
                    }
                    if (attribute.key === "VS_BENAJMIN_MLV-206") {
                        document.getElementById("tspan172-3-7-1-2-1").textContent = newValue;
                    }
                    if (attribute.key === "VS_TINAJAS_MLV-226") {
                        document.getElementById("tspan172-3-7-1-2-3").textContent = newValue;
                    }
                    if (attribute.key === "CHI_MLV-249") {
                        document.getElementById("tspan172-3-5-2-2-0-6-3-3").textContent = newValue;
                    }
                    if (attribute.key === "VS_SELVA_MLV-275") {
                        document.getElementById("tspan172-3-7-1-2-2-2").textContent = newValue;
                    }
                    if (attribute.key === "VS_PESQUERIA_MLV-296") {
                        document.getElementById("tspan172-3-7-1-2-1-4").textContent = newValue;
                    }
                    if (attribute.key === "VS_SANPEDRO_MLV-319") {
                        document.getElementById("tspan172-3-7-1-2-3-5").textContent = newValue;
                    }
